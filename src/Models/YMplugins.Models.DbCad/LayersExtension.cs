...
namespace YMplugins.Models.DbCad
{
    public static class LayersExtension
    {
        ...
        // Метод для получения слоев из одного документа
        public static List<string> GetLayers(Database? db = null)
        {
            var layers = new List<string>();

            // Используем базу данных активного документа, если не указана другая
            db ??= Application.DocumentManager.MdiActiveDocument.Database;

            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                if (db.LayerTableId.IsValid && tr.GetObject(db.LayerTableId, OpenMode.ForRead) is LayerTable lt)
                {
                    foreach (ObjectId layerId in lt)
                    {
                        ...
                        {
                            // Игнорируем ошибки при доступе к слою
                        }
                    }
                }
                tr.Commit();
        ...
        // Метод для получения слоев из всех открытых документов
        public static Dictionary<string, List<string>> GetLayersFromAllOpenDocuments()
        {
            var documentLayers = new Dictionary<string, List<string>>();

            foreach (Document acDoc in Application.DocumentManager)
            {
                // Блокируем документ и вызываем метод для конкретного документа
                using (acDoc.LockDocument())
                {
                    List<string> layers = GetLayers(acDoc.Database);
                    documentLayers.Add(acDoc.Name, layers);
                }
            }
